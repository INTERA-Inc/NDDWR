...
def load_precip_for_rch() -> pd.DataFrame:
    ...
    precip = pd.read_csv(
        os.path.join('data', 'raw', 'PRISM_precip',
                     'PRISM_ppt_provisional_4km_197001_202501_47.8746_-98.5477.csv'),
...
def build_annual_regression_from_swb_csv(
    ...
    # Try removing the low infiltration years for now...
    # !!! TODO !!!
    # annual = annual.loc[annual.index > pd.Timestamp("01-01-2010")]
...
def plot_infiltration_regression(
    ...
    show: bool = False):
    x = annual[col_precip].to_numpy()
    y = annual[col_infil].to_numpy()
    fig, ax = plt.subplots(figsize=(7, 5))
    ax.scatter(x, y, s=30, label="years")
    ...
    ax.set_xlabel("Annual precipitation (in)")
    ax.set_ylabel("Annual infiltration (in)")
    ax.set_title(title or "Annual Infiltration vs Precipitation")
    ax.legend()
    ax.grid(alpha=0.3)
    ax.text(0.02, 0.98,
...
def predict_annual_infiltration(
    ...
    yhat = regression.model.predict(X)
...
def build_yearly_rch_arrays(
    ...
    for ts, annual_depth_in in annual_infil_pred.items():
        ...
        annual_depth_in = annual_depth_in * 660 * 660  * nrow * ncol  * (12**2) # x cell volume in feet, ft2 -> in2, x # of cells
        ...
        per_cell_depth_in = per_cell_depth_in / (660 * 660 * (12**2))
...
def make_rch_from_swb(
        ...
        years_to_build: Tuple[int, int] = (1970, 1999),
    ) -> Dict[int, np.ndarray]:
    """
    ...
    print(f"Regression: I = {reg.coef:.4f} * P + {reg.intercept:.4f}  (R^2={reg.r2:.3f}, n={reg.n_years})")
...
def plot_monthly_infil_climatology(
    ...
    monthly_rate = da.resample(time="MS").mean("time")  # in/day
    ...
    fig1, axes = plt.subplots(nrows=12, ncols=2, figsize=(12, 48), constrained_layout=True)
    for m in range(1, 13):
    ...
    global_ylim = float(np.nanmax(monthly_df.values)) * 1.05 if np.isfinite(np.nanmax(monthly_df.values)) else 1.0
...
def monthly_from_netcdf(
    ...
    if make_plots:
        out_pdf = os.path.join('figures','swb2_rch.pdf')
...
def main():
    ...
    swb_csv_path = os.path.join("data","analyzed","water_balance_daily_in_day.csv")
    nc_path = os.path.join("data","analyzed","swb2_net_infiltration__2000-01-01_to_2023-12-31__175_by_156.nc")
    ...
    rch_by_year = make_rch_from_swb(
        ...
        years_to_build=(1970, 1999),
    )
    
    fig,ax = plt.subplots()
    ax.plot(sorted(rch_by_year.keys()), [np.nansum(rch_by_year[y])*365 for y in sorted(rch_by_year.keys())],
            marker='o', linestyle='-')
    ...
    fig,ax = plt.subplots()
    ax.plot(sorted(post_2000_monthly.keys()), [np.nanmean(post_2000_monthly[y]) for y in sorted(post_2000_monthly.keys())],
            marker='o', linestyle='-')

    # Optional checks
    audit_monthlies(post_2000_monthly, 2009)
