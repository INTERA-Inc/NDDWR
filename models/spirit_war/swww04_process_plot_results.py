...
def plot_fancy_obs_v_sim(m_d, obsdict, modnm='swww',plot_hdiff=False,plt_noise=True,plt_pr=True):
...
def model_packages_to_shp(d='.'):
    ...
    ghb_edge = ghb_edge[ghb_edge.iloc[:, 3:].sum(axis=1) != 0]
    ghb_edge = ghb_edge.set_crs('epsg:2265', allow_override=True)
    ghb_edge.to_file(os.path.join(o_d, 'ghb_edge.shp'), crs='epsg:2265')
    ...
    # wels
    for i in range(len(mf.wel)):
        nam = mf.wel[i].name[0]
        print(f'Saving {nam.upper()} well package')
        mf.wel[i].export(os.path.join(o_d,f'{nam}_wel.shp'), epsg=epsg)
...
if __name__ == '__main__':
    ...
    m_d = 'master_flow_barrier_rivCell_zones'
    modnm='swww'
    ...
    if plot_ies:
        ...
        plot_fancy_obs_v_sim(m_d,obsdict)
