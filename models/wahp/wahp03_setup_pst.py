...
def set_initial_array_vals(org_d, modnm='wahp7ly', run_tag=True):
    ...
    ss[3] = np.where(zon_arr[3]==5, 5e-5, ss[3])
    k[3] = np.where(zon_arr[3]==5, 0.0001, k[3])

    # plot each zone:
    # for zn in np.arange(0, np.shape(zon_arr)[0]):
    #     fig, ax = plt.subplots(figsize=(10, 10))
    #     ax.imshow(zon_arr[zn, :, :], cmap='viridis')
    #     plt.title(f'Zone Array for Layer {zn + 1}')
    #     plt.xlabel('Column Index')
    #     plt.ylabel('Row Index')
    #     # colorbar
    #     cbar = plt.colorbar(ax.images[0], ax=ax, orientation='vertical')
    #     cbar.set_label('Zone', rotation=270, labelpad=20)
    #     #plt.savefig(os.path.join(org_d, f'zone_array_lay{zn+1}.png'), dpi=300)
    #     #plt.close(fig)
    ...
    ss[4] = np.where(zon_arr[4,:,:]==5,1e-6,ss[4,:,:])
    k[4,:,:] = 175.0
...
def setup_pstpp(org_d,modnm,run_tag,template,flex_con=False,num_reals=96,
        ...
        for k,f in zip(lays,files):
            ...
            if 'k33' in f:
                ...
                if nlay == 6:
                    ...
                    if k==6:
                        pf.add_parameters(f, par_type='constant', upper_bound=2, lower_bound=0.001,
                                ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k],initial_value=0.1)
                    # elif k ==5:
                    #     pf.add_parameters(f, par_type='zone', upper_bound=100, lower_bound=0.001,
                    #                         ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                    #                         pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                    else:
                        pf.add_parameters(f, par_type='constant', upper_bound=2, lower_bound=0.001,
                                          ult_ubound=ubnds[1], ult_lbound=ubnds[0],
                ...
                if nlay == 6:
                    ...
                    if (k!=4) and (k!=0):
                        ...
                        pf.add_parameters(f, par_type='constant', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                ult_ubound=25.0, ult_lbound=0.00001, par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])  
    ...
    if flex_con:  
        ...
        pf.add_py_function('wahp03_setup_pst_nozon.py', 'process_listbudget_obs()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_nozon.py', 'znbud_by_ly_process()', is_pre_cmd=None)
        ...
        #pf.mod_sys_cmds.append('mf6')
        #pf.add_py_function('wahp03_setup_pst_nozon.py', 'interp_engine_ghb()', is_pre_cmd=True)
        ...
        pf.add_py_function('wahp03_setup_pst_nozon.py','budget_process()',is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_nozon.py', 'head_targets_process()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_nozon.py', 'dd_targets_process()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_nozon.py', 'process_head_diff_obs()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_nozon.py', 'riv_drn_bot_chk()', is_pre_cmd=True)
        # add call to processing script to pst forward run
    ...
    # add post process function to forward run script
    pf.add_py_function('wahp03_setup_pst_nozon.py', 'process_mfinput_obs()', is_pre_cmd=None)
    # add call to processing script to pst forward run
    pf.post_py_cmds.append("process_mfinput_obs('{0}')".format(modnm))
    
    pf.add_py_function('wahp03_setup_pst_nozon.py','run_2x_mf6()',is_pre_cmd=None)
...
if __name__ == "__main__":
    ...
    phi_factor_dict = {'sswbvhds':1e-20,'wbvtranshds':0.4,'wstranshds':0.2,'sswbvpmphds':1e-20,'wsssshds':1e-20,
    ...
    if high_dimensional:
        m_d_flow += '_highdim_plzgawd'
        t_d_flow += '_highdim_plzgawd'
    else:
        m_d_flow += '_lowdim'
        t_d_flow += '_lowdim'
