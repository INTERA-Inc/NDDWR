...
def set_initial_array_vals(org_d, modnm='wahp7ly', run_tag=True):
    ...
    k[3] = np.where(zon_arr[3]==5, 0.01, k[3])
...
def setup_pstpp(org_d,modnm,run_tag,template,flex_con=False,num_reals=96,
    ...
    # ------------------------------------
    pp_space = 4
    pp_vario = pyemu.geostats.ExpVario(contribution=1.0,a=5280*3.5,bearing=115.0)
    pp_geo = pyemu.geostats.GeoStruct(variograms=pp_vario)

    wbv_pp_space = 4
    wbv_vario = pyemu.geostats.ExpVario(contribution=1.0,a=5280*2.5,bearing=115.0)
    ...
    if flex_con:  
        ...
        pf.add_parameters(riv_files, par_type='constant', index_cols=[0, 1, 2], use_cols=[3], pargp='rivstg-cn',
                            ...
                            upper_bound=30 , #rivstg_cn['ubound'].values[0],
                            lower_bound=-30, # rivstg_cn['lbound'].values[0],
                            initial_value=15.0,transform="none",
                            mfile_skip=0)
        ...
        pf.add_parameters(otter_files, par_type='constant', index_cols=[0, 1, 2], use_cols=[3], pargp='otstg-cn',
                            ...
                            upper_bound=30, #otstg_cn['ubound'].values[0], #55
                            lower_bound=-20, #otstg_cn['lbound'].values[0], #-5
                            initial_value=5.0,transform="none",
                            mfile_skip=0)
        ...
        pf.add_parameters(filenames=ghb_files, par_type="constant",
                          ...
                          upper_bound=15, lower_bound=-15,initial_value = 0.0,
        ...
        pf.add_parameters(filenames=ghb_files, par_type="constant",
                          ...
                          upper_bound=300.0, lower_bound=0.1,
                          ult_ubound=20000 , ult_lbound=1000,
                          mfile_skip = 1,) #edge conductance so low
...
if __name__ == "__main__":
    ...
    phi_factor_dict = {'sswbvhds':1e-20,'wbvtranshds':0.4,'wstranshds':0.2,'sswbvpmphds':1e-20,'wsssshds':1e-20,
                        'sspmphds': 1e-20, 'transearlyhds': 0.20,'hdiff': 0.1,'ddtrgs':0.1}
    ...
    if high_dimensional:
        m_d_flow += '_highdim_dawg'
        t_d_flow += '_highdim_dawg'
    else:
        m_d_flow += '_lowdim'
        t_d_flow += '_lowdim'
