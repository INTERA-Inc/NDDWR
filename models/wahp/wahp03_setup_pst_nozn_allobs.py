...
def init_gw_vista_ss_mods(org_d,mwd,gwf=[],modnm='wahp'):
    ...
    odf = pd.read_csv(os.path.join(d,f'{modnm}.obs_continuous_wahp.ss_head.obs.output.txt'),delim_whitespace=True,header=None)
    ...
    odf.to_csv(os.path.join(d,f'{modnm}.obs_continuous_wahp.ss_head.obs.output.txt'), sep=' ', index=False, header=False)
...
def set_initial_array_vals(org_d, modnm='wahp', run_tag=True):
    # load model
    ...
    k[4,:,:] = wbv_kh_interp*0.6
    ss[4] = np.where(zon_arr[4,:,:]==5,1e-5,ss[4,:,:])
...
def run_2x_mf6(modnm='wahp'):
    ...
    keep = [ln for ln in lines if f"wahp.trans_head.obs.output" not in ln]
    ...
    content = (
        ...
        "  AUTO_FLOW_REDUCE_CSV FILEOUT wahp.sspmp.autoreduce.csv\n"
        "END options\n\n"
        "BEGIN dimensions\n"
        "  MAXBOUND  5 \n"
        "END dimensions\n\n"
...
def head_targets_process(modnm='wahp'):
    ...
    df = pd.read_csv(f"wahp.ss_head.obs.output",index_col=0)
    df.index = pd.to_datetime(start_datetime) + pd.to_timedelta(df.index.values,unit='d')
    ...
    df = pd.read_csv(f"wahp.trans_head.obs.output",index_col=0)
    df.index = pd.to_datetime(start_datetime) + pd.to_timedelta(df.index.values,unit='d')
    df.columns = [c.lower().replace(".","-") for c in df.columns]
    df.index.name = "datetime"
    ...
    df = pd.read_csv(f"wahp.sspmp_head.obs.output",index_col=0)
    df.index = pd.to_datetime(start_datetime) + pd.to_timedelta(df.index.values,unit='d')
    df.columns = [c.lower().replace(".","-") for c in df.columns]
    df.index.name = "datetime"
...
def setup_pstpp(org_d,modnm,run_tag,template,flex_con=False,num_reals=96,
        ...
        for k,f in zip(lays,files):
            ...
            if 'k33' in f:
                ...
                if nlay == 6:
                    ...
                    if k==6:
                        pf.add_parameters(f, par_type='constant', upper_bound=100, lower_bound=0.001,
                                ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k],initial_value=0.1)
                    # elif k ==5:
                ...
                if nlay == 6:
                    ...
                    if k != 4:
                        ...
                        pf.add_parameters(f, par_type='constant', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                ult_ubound=300.0, ult_lbound=5.0, par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])                        
                    # else:
    ...
    if flex_con:  
        ...
        pf.add_parameters(riv_files, par_type='constant', index_cols=[0, 1, 2], use_cols=[3], pargp='rivstg-cn',
                            ...
                            upper_bound=10 , #rivstg_cn['ubound'].values[0],
                            lower_bound=-10, # rivstg_cn['lbound'].values[0],
            ...
            if high_dimensional:
                for list_file in list_files:
                    ...
                    pf.add_parameters(filenames=list_file, par_type='constant',
                                      ...
                                      upper_bound=1.5, lower_bound=0.5, initial_value=1.0)#, datetime=dts[kper], geostruct=dom_temporal_gs)
        ...
        pf.add_py_function('wahp03_setup_pst_sspmp.py', 'process_listbudget_obs()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_sspmp.py', 'znbud_by_ly_process()', is_pre_cmd=None)
        ...
        hdiff = init_head_diff_obs(template,modnm='wahp7ly')
        pf.add_observations('sim_hdiff.csv', index_cols=['datetime'], use_cols=hdiff.columns.to_list(), obsgp='hdiff', ofile_sep=',',
                             prefix='hdiff')
        
        pf.tmp_files.append("wahp.ss_head.obs.output")
        pf.tmp_files.append("wahp.trans_head.obs.output")
        pf.tmp_files.append("wahp.sspmp_head.obs.output")
        ...
        pf.add_py_function('wahp03_setup_pst_sspmp.py','budget_process()',is_pre_cmd=None)
        #pf.add_py_function('wahp03_setup_pst_sspmp.py','riv_flux_process()',is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_sspmp.py', 'head_targets_process()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_sspmp.py', 'process_head_diff_obs()', is_pre_cmd=None)
        
        pf.add_py_function('wahp03_setup_pst_sspmp.py', 'riv_drn_bot_chk()', is_pre_cmd=True)
        # add call to processing script to pst forward run
    ...
    # add post process function to forward run script
    pf.add_py_function('wahp03_setup_pst_sspmp.py', 'process_mfinput_obs()', is_pre_cmd=None)
    # add call to processing script to pst forward run
    pf.post_py_cmds.append("process_mfinput_obs('{0}')".format(modnm))
    
    pf.add_py_function('wahp03_setup_pst_sspmp.py','run_2x_mf6()',is_pre_cmd=None)
...
def set_obsvals_and_weights(template_d,modnm="wahp",
    ...
    if flow_weight_scheme is not None:
        ...
        if flow_weight_scheme == 'all_wl_meas':
            for oname,vals in oname_obsval_dict.items():
                ...
                obs.loc[:,'count'] = len(vals)
                obs.loc[oname,'weight'] = 1.0
                if int(obs.loc[oname,'k']) == 4: # ToDo hardcoded causing issues if/then dependent upon layer
            ...
            for oname,vals in oname_obsval_dict_trans.items():
                ...
                obs.loc[oname,'observed'] = True
                obs.loc[oname,'standard_deviation'] = 1.0
                obs.loc[:,'count'] = len(vals)
                obs.loc[oname,'weight'] = 1.0
                if year >= 1980:
        ...
        for group in groups:
            ...
            quantile = np.quantile(gobs.obsval,0.25)
            gobs = gobs.loc[gobs.obsval<=quantile,:]
            obs.loc[gobs.obsnme,"weight"] *= 3
        #trobs["id"] = trobs["id"]
        ...
        obs.loc[nztrobs.loc[nztrobs.obsval>940,"obsnme"],"weight"] *= 3.0
...
def run_ies(template_ws='template_d', modnm='wahp', m_d=None, num_workers=12,niceness=False, noptmax=-1, num_reals=None,
    ...
    #pst.pestpp_options["ies_n_iter_reinflate"] = [-2,-4,999]
...
if __name__ == "__main__":
    ...
    #phi_factor_dict = { "wbvsshds":0.16,"transhds":0.33,'wbvsspmphds':0.33,'wsssshds':0.16}
    phi_factor_dict = { "sswbvhds":1e-20,"transhds":0.45,'sswbvpmphds':1e-20,'wsssshds':1e-20,
                        'sspmphds': 1e-20, 'transearlyhds': 0.37,'hdiff': 0.15}


    if high_dimensional:
        m_d_flow += "_highdim_nozn_allobs"
        t_d_flow += "_highdim_nozn_allobs"
    else:
        m_d_flow += "_lowdim"
        t_d_flow += "_lowdim"
