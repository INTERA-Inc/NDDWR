...
def plot_obs_v_sim_pmp(m_d, modnm='wahp'):
    from matplotlib.colors import ListedColormap
    sim = pd.read_csv(os.path.join(m_d,f'wahp.sspmp_head.obs.output')).T
    sim = sim.reset_index()
    sim = sim.iloc[1:,:]
    ...
    obs = pd.read_csv(os.path.join(m_d,'targets_pumping.csv'))
    obs.loc[obs.Layer >= 4, 'Layer'] = obs.loc[obs.Layer >= 4, 'Layer'] + 1

    base0 = pd.read_csv(os.path.join(m_d,f'{modnm}.base.obs.csv'))
    # get cols with SSPMP in column name:
    bcols = [col for col in base0.columns if 'sspmp_' in col]

    sshds =  base0[bcols].copy()
    # filter down sim to obsnmes that start with 'SSPMP_'
    sim = sim[sim['obsnme'].str.startswith('SSPMP_')]

    # format for obsnme that need to be added to obs: SSPMP_ID:1_K:3_I:124_J:47
    obs.columns = obs.columns.str.lower()
    obs['obsnme'] = obs.apply(lambda x: f"SSPMP_ID:{int(x['name'])}_K:{int(x['layer'])-1}_I:{x['row']-1}_J:{int(x['column'])-1}", axis=1)

    obsvsim = obs.merge(sim, on='obsnme', how='left')

    obsvsim = obsvsim.rename(columns={'target':'gwe_ft'})
    ...
    for kval, idx in k_to_idx.items():
        ax.scatter([], [], color=disc_cmap(idx), label=f'layer = {int(kval)+1}', s=50)
    ax.legend(title='Layer', bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.grid(True, linestyle='-', alpha=0.5)
    plt.tight_layout()
    plt.savefig(os.path.join(m_d, 'obs_vs_sim_heads_pumping.png'), dpi=300)
    plt.close(fig)


def plot_obs_v_sim_nopmp(m_d, modnm='wahp'):
    from matplotlib.colors import ListedColormap
    sim = pd.read_csv(os.path.join(m_d, f'wahp.ss_head.obs.output')).T
    sim = sim.iloc[:,0]
    sim = sim.reset_index()
    sim = sim.iloc[1:, :]
    sim.columns = ['obsnme', 'sim_head']
    ...
    sim['id'] = sim['id'].astype(int)
    sim['k'] = sim.obsnme.apply(lambda x: int(x.split(':')[2].split('_')[0]))
    obs = pd.read_csv(os.path.join(m_d, 'targets_nopumping.csv'))
    obs.loc[obs.Layer >= 4, 'Layer'] = obs.loc[obs.Layer >= 4, 'Layer'] + 1

    base0 = pd.read_csv(os.path.join(m_d, f'{modnm}.base.obs.csv'))
    # get cols with SSPMP in column name:
    bcols = [col for col in base0.columns if 'sshds_' in col]

    sshds = base0[bcols].copy()
    # filter down sim to obsnmes that start with 'SSPMP_'
    sim = sim[sim['obsnme'].str.startswith('SSH_')]

    # format for obsnme that need to be added to obs: SSPMP_ID:1_K:3_I:124_J:47
    obs.columns = obs.columns.str.lower()
    obs['obsnme'] = obs.apply(
        lambda x: f'SSH_ID:{int(x["name"])}_K:{int(x["layer"]) - 1}_I:{x["row"] - 1}_J:{int(x["column"]) - 1}',
        axis=1)
    ...
    ax.grid(True, linestyle='-', alpha=0.5)
    plt.tight_layout()
    plt.savefig(os.path.join(m_d, 'obs_vs_sim_heads_nopumping.png'), dpi=300)
    plt.close(fig)


def make_new_zone(mws='.',gwf=[]):
    zone = pd.read_csv(os.path.join(mws,'zones.dat'),delim_whitespace=True,header=None)
    zone.columns = ['row','col','ly','zone']

    # make layers 1 and 2 have seperate zones:
    zone.loc[(zone['ly'] == 1) , 'zone'] = 2
    zone.loc[(zone['ly'] == 2) , 'zone'] = 3
    zone.loc[(zone['ly'] == 3), 'zone'] = 4

    # make layer 3 have zones 4 and 5:
    zone.loc[(zone['ly'] == 3) ] = 4
    # change windows in layer 5:
    zone.loc[(zone['ly'] == 5) & (zone['zone'] == 4), 'zone'] = 5



    dis = gwf.dis
    id = dis.idomain.array
    nlay = dis.nlay.data
    
    if nlay == 7:
        # need to shift ly 5 zone 5 down a layer
        zone.loc[(zone['ly'] == 6), 'ly'] = 7
        zone.loc[(zone['ly'] == 5), 'ly'] = 6
        zone.loc[(zone['ly'] == 6) & (zone['zone'] == 5), 'zone'] = 6

        # need to add an new zone layer for the confining unit (ly 4), will do so
        # by duplicating the layer 3 zones:
        nzn = zone.loc[(zone['ly'] == 2)]
        nzn['ly'] = 4
        # nzn.loc[nzn['zone'] != 99, 'zone'] = 5
        zone = pd.concat([zone, nzn], ignore_index=True)
...
def init_gw_vista_ss_mods(org_d,mwd,gwf=[],modnm='wahp'):
    ...
    zon_arr = make_new_zone(gws,gwf=gwf)
    # for zn in np.arange(0,np.shape(zon_arr)[0]):
    #     fig, ax = plt.subplots(figsize=(10, 10))
    #     ax.imshow(zon_arr[zn,:,:], cmap='viridis')

    nlay = np.shape(zon_arr)[0]

    # change hk outside of window in layer 1 and 2:
    hk_array[0,:,:] = 10
    hk_array[1,:,:] = 10
    vka_array[0,:,:] = 1
    vka_array[1,:,:] = 1
    ...
    if nlay == 7:
        # need to add in another layer for the confining unit, after the first 3 layers
        hk_array = np.concatenate([hk_array[:3,:,:], np.full_like(hk_array[2,:,:], 1)[np.newaxis,:,:], hk_array[3:,:,:]], axis=0)
        vka_array = np.concatenate([vka_array[:3,:,:], np.full_like(vka_array[2,:,:], 1e-4)[np.newaxis,:,:], vka_array[3:,:,:]], axis=0)
        hk_array[2,:,:] = 10
        hk_array[3,:,:] = 1 # confining unit
        vka_array[2,:,:] = 1
        vka_array[3,:,:] = 0.1

        # increase wbv to 100
        hk_array[4, :, :] = np.where(hk_array[4, :, :] == 70, 100, hk_array[4,:,:])
        vka_array[4, :, :] = np.where(vka_array[4, :, :] == 0.3, 10, vka_array[4, :, :])
    ...
    # with open(os.path.join(d,f"{modnm}.nam"), "r") as f:
    #     lines = f.readlines()
    # keep = [ln for ln in lines if not ln.lstrip().startswith(("GHB6", "RCH6"))]
    # with open(os.path.join(d,f"{modnm}.nam"), "w") as f:
    #     f.writelines(keep)
    ...
    def upf_to_npf(w_d,hk_array,aniso_array):
        ...
        for file in kh_files:
            ...
            if len(kh_files) == 6:
                if ly == 4:
                    # load idomain for layer 4:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer4.txt'))
                    hk = np.where(idom != -1, np.max(hk_array[ly-1,:,:]), 0.0001)
                elif ly == 6:
                    # load idomain for layer 6:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer6.txt'))
                    hk = np.where(idom > 0, np.max(hk_array[ly-1,:,:]), 0.0001)
                else:
                    hk = hk_array[ly-1,:,:]
            elif len(kh_files) == 7:
                if ly == 5:
                    # load idomain for layer 5:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer5.txt'))
                    hk = np.where(idom != -1, np.max(hk_array[ly-1,:,:]), 0.0001)
                elif ly == 7:
                    # load idomain for layer 7:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer7.txt'))
                    hk = np.where(idom > 0, np.max(hk_array[ly-1,:,:]), 0.0001)
                else:
                    hk = hk_array[ly-1,:,:]
                
            np.savetxt(os.path.join(w_d, file), hk, fmt='%15.6E')
        ...
        for file in aniso_files:
            ...
            if len(aniso_files) == 6:
                if ly == 4:
                    # load idomain for layer 4:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer4.txt'))
                    vka = np.where(idom != -1, np.max(aniso_array[ly-1,:,:]), 0.0001)   
                elif ly == 6:
                    # load idomain for layer 6:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer6.txt'))
                    vka = np.where(idom > 0, np.max(aniso_array[ly-1,:,:]), 0.0001)
                else:
                    vka = aniso_array[ly-1,:,:]
            elif len(aniso_files) == 7:
                if ly == 5:
                    # load idomain for layer 5:
                    idom = np.loadtxt(os.path.join(w_d, 'dis_idomain_layer5.txt'))
                    vka = np.where(idom != -1, np.max(aniso_array[ly-1,:,:]), 0.0001)   
            ...
            np.savetxt(os.path.join(w_d, file), vka, fmt='%15.6E')
    
    upf_to_npf(d,hk_array,aniso_array)
    

    # kh_files = [f for f in os.listdir(d) if f.startswith('npf_k_')]
    # kh_files.sort(key=lambda x: int(x.split('_')[2].split('.')[0].replace('layer','')))
    
    # init_kh = {}
    # for file in kh_files:
    #     kh = np.loadtxt(os.path.join(d, file))
    #     fig, ax = plt.subplots(figsize=(10, 10))
    #     ax.imshow(kh, cmap='viridis')
    #     plt.title(f'Hydraulic Conductivity Layer {file.split("_")[2].split(".")[0].replace("layer","")}')
    #     # colorbar
    #     cbar = plt.colorbar(ax.images[0], ax=ax, orientation='vertical')
    # vk_files = [f for f in os.listdir(d) if f.startswith('npf_k33')]
    # vk_files.sort(key=lambda x: int(x.split('_')[2].split('.')[0].replace('layer','')))
    # init_vka = {}
    # for file in vk_files:
    #     vk = np.loadtxt(os.path.join(d, file))
    #     fig, ax = plt.subplots(figsize=(10, 10))
    #     ax.imshow(vk, cmap='viridis')
    #     plt.title(f'Hydraulic Conductivity Layer {file.split("_")[2].split(".")[0].replace("layer","")}')
    #     # colorbar
    #     cbar = plt.colorbar(ax.images[0], ax=ax, orientation='vertical')
    ...
    sim = flopy.mf6.MFSimulation.load(sim_ws=d, exe_name='mf6',load_only=['dis'])
    m = sim.get_model(f'{modnm}')
    idom = m.dis.idomain.array

    # modify obs package to include the new targets:
    odf = pd.read_csv(os.path.join(d,f'{modnm}.obs_continuous_{modnm}.ss_head.obs.output.txt'),delim_whitespace=True,header=None)
    ...
    if nlay == 7:
        # increase layer from layer wbv and deeper
        targs_nopmp.loc[targs_nopmp.layer>=4, 'layer'] = targs_nopmp.loc[targs_nopmp.layer>=4, 'layer'] + 1
        targs_pmp.loc[targs_pmp.layer >= 4,'layer'] = targs_pmp.loc[targs_pmp.layer >= 4, 'layer'] + 1
    ...
    for _, row in targs_nopmp.iterrows():
        ...
        if not ((odf.ly == ly) & (odf.row == row_num) & (odf.col == col_num)).any():
            ...
            # check if in idom:
            if idom[ly-1,row_num-1,col_num-1] > 0:
    ...
    odf.to_csv(os.path.join(d,f'{modnm}.obs_continuous_{modnm}.ss_head.obs.output.txt'), sep=' ', index=False, header=False)
    ...
    mfiles = [f'{modnm}.oc', f'{modnm}.obs', f'{modnm}.nam', f'{modnm}.tdis']
    ...
    # copy over zone budget by layer file cause somehow its disappeared:
    shutil.copy2(os.path.join(org_d,'zones_byly.dat'), os.path.join(d,'zones_byly.dat'))


def set_initial_array_vals(org_d, modnm='wahp7ly', run_tag=True):
    # load model
    sim = flopy.mf6.MFSimulation.load(sim_ws=org_d, exe_name='mf6', )
    m = sim.get_model(modnm)
    
    # get zones
    zon_arr = make_new_zone(org_d, gwf=m)
    k = m.npf.k.array
    aniso = m.npf.k33.array
    ss = m.sto.ss.array
    sy = m.sto.sy.array
    riv = m.riv.stress_period_data.data[0]
    riv_df = pd.DataFrame(riv, columns=['layer', 'row', 'col', 'stage', 'cond', 'rbot'])

    riv_df[['layer', 'row', 'col']] = pd.DataFrame(riv['cellid'].tolist(), index=riv_df.index)
    riv_df[['layer','row','col']] = riv_df[['layer','row','col']].astype(int)
    ...
    # update upper confining unit:
    sy[3] = np.where(zon_arr[3]==5, 0.05, sy[3])
    ss[3] = 1e-4
    k[3] = 0.05
    aniso[3] = 0.001

    
    # update lower confining unit:
    # k[5] = np.where(zon_arr[5,:,:] == 6, 0.01, k[5,:,:])
    # ss[5] = np.where(zon_arr[5,:,:] == 6, 1.5e-4, ss[5,:,:])
    # sy[5] = np.where(zon_arr[5,:,:] == 6, 0.05, sy[5,:,:])
    k[5] = 0.01
    ss[5] = 1.5e-4
    sy[5] = 0.05
    aniso[5] = 0.01
    ...
    # update WR:
    k[6] = np.where(k[6,:,:] == 500, 75, k[6,:,:])
    ss[6] = np.where(zon_arr[6,:,:]==7,1e-5,ss[6,:,:])
    aniso[6] = 0.01
    
    # update wbv:
    #wbv_kh_interp = np.loadtxt(os.path.join('data','analyzed','wbv_interp_results.txt'))
    #k[4,:,:] = wbv_kh_interp*0.8
    ss[4] = 1e-5
    k[4,:,:] = 175.0

    #k[0] = np.where(k[0,:,:] > 10, k[0,:,:]*.05, k[0,:,:])
    ...
    if run_tag:
        ...
        curdir = os.getcwd()
        os.chdir(org_d)
        znbud_by_ly_process(modnm='wahp7ly')
        os.chdir(curdir)
        #wpp.model_packages_to_shp_joined(d=org_d, debug=True, bc_first_sp_only=True)


def run_2x_mf6(modnm='wahp'):
    import re
    ...
    # remove wel file: don't remove stress period data file so we can update parameter
    wel_file = f'{modnm}.sspmp'
    ...
    '''
      ...
      3. Add otriv file for SS pumping
      3. need to change .nam file to add the pumping stress period data file and otriv sspmp
    ...
    with open(f"{modnm}.obs", "r") as f:
        text = f.read()
    text = text.replace(f" {modnm}.ss_head", f" {modnm}.sspmp_head")
    with open(f"{modnm}.obs", "w") as f:
        f.write(text)
    ...
    keep = [ln for ln in lines if f"{modnm}.trans_head.obs.output" not in ln]
    with open(fname, "w") as f:
        f.writelines(keep)
        
    # build the pumping file and add to .nam file: #ToDo should this be layer 5?
    #ToDo sav fix based on NLAY
    fnm = 'sspmp_stress_period_data_1.txt'
    # if file exists, continue
    if os.path.exists(fnm):
        print(f"{fnm} already exists, skipping creation.")
    ...
    content = (
        ...
        f"  AUTO_FLOW_REDUCE_CSV FILEOUT {modnm}.sspmp.autoreduce.csv\n"
        "END options\n\n"
        "BEGIN dimensions\n"
        "  MAXBOUND  5 \n"
        "END dimensions\n\n"
    ...
    # now modify .nam to include new well and riv pkg:
    fname = f"{modnm}.nam"
    ...
    for i, line in enumerate(lines):
        if line.lstrip().startswith("END packages"):
            new_line = f"  WEL6  {modnm}.sspmp  sspmp\n"
...
def init_2x_mf6(d,modnm='wahp'):
    b_d = os.getcwd()
    os.chdir(d)
    dfs = run_2x_mf6(modnm=modnm)
    os.chdir(b_d)
...
def riv_flux_process(modnm='wahp'):
...
def init_riv_process(d,modnm='wahp'):
    b_d = os.getcwd()
    os.chdir(d)
    dfs = riv_flux_process(modnm=modnm)
    os.chdir(b_d)
    return dfs

    
def budget_process(modnm='wahp'):
...
def init_budget_process(d,modnm='wahp7ly'):
    b_d = os.getcwd()
    os.chdir(d)
...
def head_targets_process(modnm='wahp'):
    
    sim = flopy.mf6.MFSimulation.load(sim_ws='.',load_only=['dis','tdis'])
    start_datetime = sim.tdis.start_date_time.array
    m = sim.get_model(modnm)

    df = pd.read_csv(f'{modnm}.ss_head.obs.output',index_col=0)
    df.index = pd.to_datetime(start_datetime) + pd.to_timedelta(df.index.values,unit='d')
    ...
    df = pd.read_csv(f'{modnm}.sspmp_head.obs.output',index_col=0)
    df.index = pd.to_datetime(start_datetime) + pd.to_timedelta(df.index.values,unit='d')
    df.columns = [c.lower().replace('.','-') for c in df.columns]
    df.index.name = 'datetime'

    df.to_csv(f'{modnm}.sspmp_head.obs.output.csv')
...
def init_head_targets_process(d,modnm='wahp'):
    b_d = os.getcwd()
    os.chdir(d)
...
def dd_targets_process(modnm='wahp7ly', filter_to_index_wells=True):
    # --- load model time and sim/obs tables ---
    sim = flopy.mf6.MFSimulation.load(sim_ws='.', load_only=['dis', 'tdis'])
    start_datetime = sim.tdis.start_date_time.array
    m = sim.get_model(modnm)
    ...
    if filter_to_index_wells:
        idx_wells = pd.read_csv('water_lvl_targs_manual_ly_assign.csv')
        idx_wells = idx_wells.loc[idx_wells['index_well_flag'] == True]
        idx_wells['grp_id'] = (
            'transh_grpid:' + idx_wells['group number'].astype(str)
            + '_k:' + (idx_wells['manually_corrected_lay'] - 1).astype(str)
            + '_i:' + (idx_wells['row']).astype(str)
            + '_j:' + (idx_wells['col']).astype(str)
        )
        cols = idx_wells['grp_id'].astype(str).tolist()
        cols.sort()
        # drop duplicates but preserve order
        cols = list(dict.fromkeys(cols))

        # keep only columns that exist in BOTH df and t_df
        cols_keep = [c for c in cols if (c in df.columns) and (c in t_df.columns)]
        if not cols_keep:
            raise ValueError('No overlapping columns between df/t_df and requested cols.')

        # filter
        obs = t_df[cols_keep].copy()
        sim = df[cols_keep].copy()

        # clip sim to obs end date, then align to obs index to guarantee same shape/order
        sim = sim[sim.index <= obs.index.max()]
        # align index (obs is the driver)
        sim = sim.reindex(obs.index)

        # same shape/columns check
        assert list(sim.columns) == list(obs.columns), "Column mismatch after filtering."
        assert sim.shape == obs.shape, f"sim and obs have different shapes: {sim.shape} vs {obs.shape}"

        # mask sim wherever obs is NaN (we don't want diffs where obs has no data)
        sim_masked = sim.where(obs.notna())

        # --- anchored difference ---
        # For each column c:
        #   t0 = first non-NaN in obs[c]
        #   base = sim_masked[c] at t0 (if NaN, use next non-NaN in sim after t0)
        #   diff = base - sim_masked[c]
        #   set times < t0_used to NaN
        df_diff = pd.DataFrame(index=sim_masked.index, columns=sim_masked.columns, dtype=float)
        
        for c in sim_masked.columns:
            first_idx = obs[c].first_valid_index()
            if first_idx is None:
                # no observation at all -> all NaN
                df_diff[c] = np.nan
                continue

            base = sim_masked.at[first_idx, c] if first_idx in sim_masked.index else np.nan
            t0_used = first_idx

            if pd.isna(base):
                # find the first valid sim value at/after the anchor time
                future = sim_masked.loc[first_idx:, c]
                t0_used = future.first_valid_index()
                if t0_used is None:
                    # no usable sim values -> all NaN
                    df_diff[c] = np.nan
                    continue
                base = sim_masked.at[t0_used, c]

            # compute difference
            col_diff = base - sim_masked[c]
            # anything before the effective anchor is NaN
            col_diff.loc[col_diff.index < t0_used] = np.nan
            df_diff[c] = col_diff


        # --- anchored difference for OBS using the SAME anchor logic ---
        obs_diff = pd.DataFrame(index=obs.index, columns=obs.columns, dtype=float)
        ...
        for c in obs.columns:
            ...
            # start with the same anchor we used for sim (t0_used),
            # but it was computed inside the sim loop. Recompute here consistently:
            t0_used = first_idx
            base_sim = sim_masked.at[first_idx, c]
            if pd.isna(base_sim):
                future_sim = sim_masked.loc[first_idx:, c]
                t0_used = future_sim.first_valid_index() or first_idx  # if sim never valid, keep first_idx

            # choose obs anchor as first non-NaN OBS value at/after t0_used
            future_obs = obs.loc[t0_used:, c]
            t0_obs_used = future_obs.first_valid_index()
            if t0_obs_used is None:
                # no usable obs after the (sim-driven) anchor
                obs_diff[c] = np.nan
                continue

            base_obs = obs.at[t0_obs_used, c]

            col = base_obs - obs[c]
            col.loc[col.index < t0_obs_used] = np.nan
            obs_diff[c] = col

        # save
        df_diff.index.name = 'datetime'
        # replace nans with -9999.0
        df_diff.fillna(-9999.0, inplace=True)
        # replace 'transh' in column names with 'dd'
        df_diff.columns = [c.replace('transh', 'dd') for c in df_diff.columns]
        obs_diff.columns = [c.replace('transh', 'dd') for c in obs_diff.columns]

        df_diff.to_csv(f'{modnm}.trans_sim_dd_targs.output')
        obs_diff.to_csv(f'{modnm}.trans_obs_dd_targs.output')

        return df_diff, obs_diff

    # if not filtering, just return raw tables
    return df, t_df


def init_dd_targets_process(d,modnm='wahp7ly'):
    b_d = os.getcwd()
    os.chdir(d)
...
def process_head_diff_obs(modnm='wahp7ly'):
    '''post processor to return head difference values from MODFLOW output files

    Args:
        mod_name (str): MODFLOW model name

    Returns:
        head_diff: Pandas DataFrame object of head differences from output files
    '''
    sim = flopy.mf6.MFSimulation.load(sim_ws=".",load_only=['dis','tdis'])
    start_datetime = sim.tdis.start_date_time.array
    m = sim.get_model(modnm)
    
    hdif = pd.read_csv(f'transient_well_head_diffs.csv')
    # set index to hdif['start_datetime']
    hdif.set_index('start_datetime', inplace=True)
    hdif.index.name = "datetime"
    ...
    simhdiff = pd.DataFrame(index=df.index)
    for col in hdif.columns:
        h1nm = '_'.join(col.split('_')[:-1]).split('-')[0].replace('.',':').replace('grp','grpid')
        h2nm = h1nm[:-1] + '_'.join(col.split('_')[:-1]).split('-')[1].replace('.',':').replace('grp','grpid')
        # get df column with h1nm and h2nm strings in name:
        h1_col = [c for c in df.columns if h1nm in c][0]
        h2_col = [c for c in df.columns if h2nm in c][0]
        hd = df[h1_col] - df[h2_col]
        simhdiff[col] = hd

    simhdiff.to_csv(f'sim_hdiff.csv')

    return simhdiff


def init_head_diff_obs(d,modnm='wahp7ly'):
    b_d = os.getcwd()
    os.chdir(d)
    hdiff = process_head_diff_obs(modnm=modnm)
    os.chdir(b_d)
    return hdiff


def process_listbudget_obs(mod_name='wahp7ly'):
...
def init_listbudget_obs(d='.', mod_name='wahp7ly'):
...
def process_mfinput_obs(mod_name='wahp7ly'):
...
def init_mfinput_obs(template_ws='template', mod_name='wahp7ly'):
...
def init_regional_ghbs(d='.', mod_name='wahp7ly'):
...
def setup_pstpp(org_d,modnm,run_tag,template,flex_con=False,num_reals=96,
                high_dimensional=False, tie_window_pars=False ):
    ...
    # init sspmp run:
    init_2x_mf6(temp_d,modnm=modnm)
    ...
    # ------------------------------------
    pp_space = 4
    pp_vario = pyemu.geostats.ExpVario(contribution=1.0,a=5280*3,bearing=115.0)
    pp_geo = pyemu.geostats.GeoStruct(variograms=pp_vario)

    wbv_pp_space = 4
    wbv_vario = pyemu.geostats.ExpVario(contribution=1.0,a=5280*2,bearing=115.0)
    wbv_pp_geo = pyemu.geostats.GeoStruct(variograms=wbv_vario)

    gr_v = pyemu.geostats.ExpVario(contribution=1.0,a=660*4,anisotropy=1.0,bearing=0.0)
    gr_gs = pyemu.geostats.GeoStruct(variograms=gr_v)
    ...
    shutil.copy2(os.path.join('gwvistas','final_ss_models_ONLY_riv','zones.dat'), template)
    zon_arr = make_new_zone(mws=template,gwf=m)
    ...
    for files, kubnds in zip(stacked_files,stacked_ubnds):
        ...
        for k,f in zip(lays,files):
            ...
            if 'k33' in f:
                ...
                if nlay == 6:
                    if (k ==3):
                        pf.add_parameters(f, par_type='constant', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                        ...
                        pf.add_parameters(f, par_type='constant', upper_bound=5, lower_bound=0.1,
                            	ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                            	pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k],initial_value=0.1)
                    else:
                        pf.add_parameters(f, par_type='zone', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                            ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                                            pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                elif nlay == 7:
                    if (k!=0) and (k!=3) and (k!=5):
                        print(f'not parameterizing k33 in k:{k}')
                        # pf.add_parameters(f, par_type='constant', upper_bound=5, lower_bound=0.001,
                        #         ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                        #         pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])
                    else:
                        pf.add_parameters(f, par_type='constant', upper_bound=10, lower_bound=0.001,
                                ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])                        

                if high_dimensional:
                    if (k!=0) and (k!=3) and (k!=5):
                        print(f'not parameterizing k33 with PP in k:{k}')
                        # pf.add_parameters(f, par_type='pilotpoints', upper_bound=5, lower_bound=0.01,
                        ...
                        pf.add_parameters(f, par_type='pilotpoints', upper_bound=10, lower_bound=0.001,
                            ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='pp-' + par_name_base+ str(k).zfill(3),
                            pargp='pp-' + par_name_base + str(k).zfill(3), pp_space=pp_space,geostruct=pp_geo)
                ...
                if nlay == 6:
                    if (k ==3) or (k==5):
                        pf.add_parameters(f, par_type='constant', upper_bound=100, lower_bound=0.01,
                                ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=id[k])
                    else:
                        pf.add_parameters(f, par_type='zone', upper_bound=100, lower_bound=0.01,
                                            ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                                            pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                elif nlay == 7:
                    if (k!=4) and (k!=0) and (k!=6):
                        pf.add_parameters(f, par_type='constant', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])
                    elif k==4:
                        pf.add_parameters(f, par_type='constant', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                ult_ubound=350.0, ult_lbound=0.01, par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])  
                    elif k==0:
                        pf.add_parameters(f, par_type='constant', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                ult_ubound=50.0, ult_lbound=0.00001, par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])  
                                          
                    # else:
                    #     pf.add_parameters(f, par_type='zone', upper_bound=100, lower_bound=0.02,
                    #                         ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                    #                         pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                if high_dimensional:
                    if (k!=4) and (k!=6):
                        pf.add_parameters(f, par_type='pilotpoints', upper_bound=100, lower_bound=0.01,
                                ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='pp-' + par_name_base,
                                pargp='pp-' + par_name_base + str(k).zfill(3), pp_space=pp_space,geostruct=pp_geo)
                        # pf.add_parameters(f, par_type='grid', upper_bound=50, lower_bound=0.1,
                        #           ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='gr-' + par_name_base,
                        #           pargp='gr-' + par_name_base + str(k).zfill(3), geostruct=gr_gs,
                        #           zone_array=id[k])
                    elif k==6:
                        print('not parameterizing WR')
                    else:
                        pf.add_parameters(f, par_type='pilotpoints', upper_bound=100, lower_bound=0.01,
                                par_name_base='pp-' + par_name_base,
                                pargp='pp-' + par_name_base + str(k).zfill(3), pp_space=wbv_pp_space,geostruct=wbv_pp_geo)
                        # pf.add_parameters(f, par_type='grid', upper_bound=50, lower_bound=0.1,
                        #           ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='gr-' + par_name_base,
                        #           pargp='gr-' + par_name_base + str(k).zfill(3), geostruct=gr_gs,
                        #           zone_array=id[k])                        
                ...
                if nlay == 6:
                    if k == 3:
                        ...
                        pf.add_parameters(f, par_type='constant', upper_bound=20.0, lower_bound=0.05,
	                            ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
	                            pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])
                    else:
                        pf.add_parameters(f, par_type='zone', upper_bound=bnds_cn[1], lower_bound=bnds_cn[0],
                                            ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                                            pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                    # pf.add_parameters(f, par_type='grid', upper_bound=10, lower_bound=0.01,
                    #               ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='gr-' + par_name_base,
                    #               pargp='gr-' + par_name_base + str(k).zfill(3), geostruct=gr_gs,
                    #               zone_array=id[k])
                    ...
                    if k!=6:
                        pf.add_parameters(f, par_type='constant', upper_bound=20.0, lower_bound=0.05,
                            ...
                            pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])
                    # else:
                    #     pf.add_parameters(f, par_type='zone', upper_bound=20.0, lower_bound=0.05,
                    #                         ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                    #                         pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                if high_dimensional:
                    if k!=6:
                        pf.add_parameters(f, par_type='pilotpoints', upper_bound=20.0, lower_bound=0.05,
                                            ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='pp-' + par_name_base+ str(k).zfill(3),
                                            pargp='pp-' + par_name_base + str(k).zfill(3),pp_space=pp_space,geostruct=pp_geo)
                    # pf.add_parameters(f, par_type='grid', upper_bound=10, lower_bound=0.01,
                    #               ult_ubound=ubnds[1], ult_lbound=ubnds[0], par_name_base='gr-' + par_name_base,
                ...
                if nlay == 6:
                    if (k ==3) or (k==5):
                        pf.add_parameters(f, par_type='constant', upper_bound=3.0, lower_bound=0.33,
                                ult_ubound=0.25, ult_lbound=ubnds[0], par_name_base='cn-' + par_name_base+ str(k).zfill(3),
                                pargp='cn-' + par_name_base + str(k).zfill(3), zone_array=id[k])
                    else:
                        pf.add_parameters(f, par_type='zone', upper_bound=3.0, lower_bound=0.33,
                                            ult_ubound=0.25, ult_lbound=ubnds[0], par_name_base='zn-' + par_name_base+ str(k).zfill(3),
                                            pargp='zn-' + par_name_base + str(k).zfill(3), zone_array=zon_arr[k])
                elif nlay == 7:
                    if k <= 1:
                        pf.add_parameters(f, par_type='constant', upper_bound=3.0, lower_bound=0.33,
                        ...
                        if high_dimensional:
                            pf.add_parameters(f, par_type='pilotpoints', upper_bound=3.0, lower_bound=0.33,
                                                    ult_ubound=0.25, ult_lbound=ubnds[0], par_name_base='pp-' + par_name_base+ str(k).zfill(3),
                                                    pargp='pp-' + par_name_base + str(k).zfill(3),pp_space=pp_space,geostruct=pp_geo)
            if 'sy' not in f and k!=6 and 'k33' not in f:
                pf.add_observations(f,obsgp=par_name_base + str(k).zfill(3),prefix=par_name_base + str(k).zfill(3))  
            elif 'sy' in f and k<=1 and k!=6:
                pf.add_observations(f,obsgp=par_name_base + str(k).zfill(3),prefix=par_name_base + str(k).zfill(3))
            elif 'k33' in f and k!=6 and k!=4 and k!=2 and k!=1:
                pf.add_observations(f,obsgp=par_name_base + str(k).zfill(3),prefix=par_name_base + str(k).zfill(3))
    ...
    if flex_con:  
        ...
        pf.add_parameters(riv_files, par_type='grid',
                    par_name_base='riv_grid', geostruct=gr_gs,
                    pargp='riv_grid', index_cols=[0, 1, 2], use_cols=[4],
                    upper_bound=100, lower_bound=0.001,mfile_skip=0,)
        ...
        #                     upper_bound=15 , #rivstg_cn['ubound'].values[0],
        #                     lower_bound=-15, # rivstg_cn['lbound'].values[0],
        #                     initial_value=0.0,transform="none",
        #                     mfile_skip=0)

        otcond_cn = par.loc[par.parm=='otcond_cn']
        otstg_cn = par.loc[par.parm=='otstg_cn']
        otter_files = [f for f in os.listdir(template) if f.startswith('otriv_stre') and f.endswith('.txt')]
        ...
        pf.add_parameters(otter_files, par_type='constant', index_cols=[0, 1, 2], use_cols=[4], pargp='otcond-cn',
                            ...
                            upper_bound=otcond_cn['ubound'].values[0], #100.0
                            lower_bound=otcond_cn['lbound'].values[0], #0.01
                            mfile_skip=0)
        pf.add_parameters(otter_files, par_type='grid',
            par_name_base='otter_grid', geostruct=gr_gs,
            pargp='otter_grid', index_cols=[0, 1, 2], use_cols=[4],
            upper_bound=100, lower_bound=0.001,mfile_skip=0,)
        ...
        #                     upper_bound=20, #otstg_cn['ubound'].values[0], #55
        #                     lower_bound=-20, #otstg_cn['lbound'].values[0], #-5
        #                     initial_value=0.0,transform="none",
        #                     mfile_skip=0)
        ...
        pf.add_parameters(filenames=drn_files, par_type='constant',
                            ...
                            mfile_skip=0)
        pf.add_parameters(drn_files, par_type='grid',
            par_name_base='drn_grid', geostruct=gr_gs,
            pargp='drn_grid', index_cols=[0, 1, 2], use_cols=[4],
            upper_bound=100, lower_bound=0.001,mfile_skip=0,)
        ...
        pf.add_parameters(rch_files, par_type='pilotpoints', upper_bound=2.0, lower_bound=0.5,
            par_name_base='rch_pp', pargp='rch_pp', geostruct=rch_pp_geo, pp_space=rch_pp_space)
        
        for rch_file in rch_files:
            sp = int(rch_file.split('.')[0].split('_')[-1])
            kper = int(sp) - 1
            dt = dts[kper]
            pf.add_parameters(rch_file,par_type='constant',pargp='rcht',par_name_base='rcht_{0:03d}'.format(kper),
                              datetime=dt,geostruct=temporal_gs,upper_bound=2,lower_bound=.5)
        ...
        # domestic water use
        #wel_packages = [f for f in os.listdir(template) if f.lower().endswith('.dom')]
        #for wel_package in wel_packages:
        #    wel_package = wel_package.replace('.','_')
        prefixes = ["cob","car","malt","minn","cow"]
                ...
                for list_file in list_files:
                    ...
                    if kper >=4 and kper <=33:
                        pf.add_parameters(filenames=list_file, par_type='constant',
                                        ...
                                        upper_bound=1.4, lower_bound=0.6, initial_value=1.0, datetime=dts[kper], geostruct=temporal_gs)
                    elif kper > 33:
                        pf.add_parameters(filenames=list_file, par_type='constant',
                                        par_name_base='twel-mlt-{1}_kper:{0:04d}'.format(sp-1,tag),
                                        pargp='twel-mlt-{0}'.format(tag), index_cols=[0, 1, 2], use_cols=[3],
                                        upper_bound=1.1, lower_bound=0.9, initial_value=1.0, datetime=dts[kper], geostruct=temporal_gs)   
        ...
        # add noise to ss pumping file
        list_files = [f for f in os.listdir(template) if f.split("_")[0] in ["sspmp"] and f.endswith('.txt') and "_stress_period_data_" in f]
        print(list_files)
        list_files.sort()
        pf.add_parameters(filenames=list_files, par_type='constant',
                              par_name_base='sspmp_wel-mlt',
                              pargp='sspmp_wel-mlt', index_cols=[0, 1, 2], use_cols=[3],
                              upper_bound=1.5, lower_bound=0.5, initial_value=1.0)
        ...
        ghb_files = [f for f in os.listdir(template) if f.startswith(f'{modnm}_wbv') and f.endswith('.txt')]
        ghb_files.sort()
        ...
        #                   par_name_base='ghbwbv_stg_' + "cn",
        #                   pargp='ghbwbv_stg_' + "gr", index_cols=[0, 1, 2],
        #                   upper_bound=30, lower_bound=-30,initial_value = 0.0,
        #                    use_cols=[3],par_style="a", mfile_skip=0,
        #                   transform="none")

        pf.add_parameters(filenames=ghb_files, par_type="constant",
                          par_name_base='ghbwbv_cond'+ "cn",
                          pargp='ghbwbv_cond' + "cn",
                          index_cols=[0, 1, 2], use_cols=[4],
                          upper_bound=100.0, lower_bound=0.1,
                          ult_ubound=3500 , ult_lbound=10,
                          mfile_skip=0,) #edge conductance so low
        ...
        pf.add_py_function('wahp03_setup_pst_noWR.py', 'process_listbudget_obs()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_noWR.py', 'znbud_by_ly_process()', is_pre_cmd=None)
        # add call to processing script to pst forward run
        pf.post_py_cmds.append("process_listbudget_obs('{0}')".format(modnm))
        
        # init sspmp run:
        init_2x_mf6(template,modnm=modnm)
        ...
        pf.add_observations(f'{modnm}.sspmp_head.obs.output.csv', index_cols=['datetime'], use_cols=hdf[2].columns.to_list(), obsgp='ssphds', ofile_sep=',',
                             prefix='sspmphds')
        ...
        hdiff = init_head_diff_obs(template,modnm=modnm)
        pf.add_observations('sim_hdiff.csv', index_cols=['datetime'], use_cols=hdiff.columns.to_list(), obsgp='hdiff', ofile_sep=',',
                             prefix='hdiff')
        ...
        pf.tmp_files.append("sim_hdiff.csv")
        ...
        #pf.mod_sys_cmds.append('mf6')
        #pf.add_py_function('wahp03_setup_pst_noWR.py', 'interp_engine_ghb()', is_pre_cmd=True)
        pf.post_py_cmds.append(f'head_targets_process(modnm="{modnm}")')
        pf.post_py_cmds.append(f'dd_targets_process(modnm="{modnm}")')
        pf.post_py_cmds.append(f'process_head_diff_obs(modnm="{modnm}")')
        ...
        pf.add_py_function('wahp03_setup_pst_noWR.py','budget_process()',is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_noWR.py', 'head_targets_process()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_noWR.py', 'dd_targets_process()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_noWR.py', 'process_head_diff_obs()', is_pre_cmd=None)
        pf.add_py_function('wahp03_setup_pst_noWR.py', 'riv_drn_bot_chk()', is_pre_cmd=True)
        # add call to processing script to pst forward run
    ...
    # add post process function to forward run script
    pf.add_py_function('wahp03_setup_pst_noWR.py', 'process_mfinput_obs()', is_pre_cmd=None)
    # add call to processing script to pst forward run
    pf.post_py_cmds.append("process_mfinput_obs('{0}')".format(modnm))
    
    pf.add_py_function('wahp03_setup_pst_noWR.py','run_2x_mf6()',is_pre_cmd=None)
    pf.mod_py_cmds.append(f"run_2x_mf6(modnm='{modnm}')")
    ...
    if tie_window_pars:
        # group edge ghbs:
        pdata = pst.parameter_data.copy()
        pcols = list(pdata.columns) + ['partied']
        zndf = pd.DataFrame(data=None, columns=pcols)
        df = pdata[pdata['zone']=='99'].copy()

        group = df.groupby(['pname',])
        for nm, grp in group:
            # sort group by idx0:
            # grp = grp.sort_values(by='idx0')
            grp['partied'] = np.nan
            grp.loc[:, 'partrans'] = 'tied'
            # tie deeper layers to upper
            grp.loc[grp.k.values.astype(int) ==0, 'partrans'] = 'log'
            # grp.loc[grp.idx0==min(grp.idx0),'partrans'] = 'none'
            tiedto = grp.loc[grp.k.values.astype(int) ==0, 'parnme']
            grp.loc[grp.partrans == 'tied', 'partied'] = tiedto.values[0]
            # grp.loc[grp.idx0==min(grp.idx0),'partied'] = np.nan
            # grp.loc[grp.partrans=='fixed','partied'] = True

            zndf = pd.concat([zndf, grp[pcols]])
        zndf.index = zndf.parnme

        # update pst parameter data with ghbdf:
        pst.parameter_data['partied'] = np.nan
        pst.parameter_data.update(zndf)
    ...
    pst.write(os.path.join(template,f'{modnm}.pst'),version=2)
    pyemu.os_utils.run(f'pestpp-ies {modnm}.pst',cwd=template)
    pst.set_res(os.path.join(template,f'{modnm}.base.rei'))
    print('phi',pst.phi)
    ...
    pst.write(os.path.join(pf.new_d, f'{modnm}.pst'),version=2)
...
def set_obsvals_and_weights(template_d,modnm="wahp7ly",
    ...
    cluster = gpd.read_file(os.path.join('..','..','gis','input_shps','wahp','cluster_obs.shp'))
    ...
    if flow_weight_scheme is not None:
        ...
        sspmp_df = pd.read_csv(os.path.join('data','analyzed','targets_pumping.csv'))
        # make obsprefix in this format: ssh_id:1_k:3_i:124_j:47

        sspmp = sspmp_df[['Target', 'Name', 'row', 'column']].copy()

        sspmp.rename(columns={'Target': 'gwe_ft'}, inplace=True)
        sspmp['datetime'] = h_df.datetime.values[0]
        sspmp['k'] = sspmp_df['Layer'].values - 1
        sspmp['i'] = sspmp_df['row'].values
        sspmp['j'] = sspmp_df['column'].values
        if nlay == 6:
            print('gwv layers same as mf6 layers')
        elif nlay == 7:
            sspmp.loc[sspmp.k>=3, 'k'] = sspmp.loc[sspmp.k>=3, 'k']+1 # update WBV and below

        sspmp['obsprefix'] = sspmp.apply(
            lambda x: 'sspmp_id:{0}_k:{1}_i:{2}_j:{3}'.format(x['Name'], x['k'], x['row']-1, x['column']-1), axis=1)

        sspmp["otype"] = "sspmp"
        h_df["otype"] = "head"
        # h_df = pd.concat([h_df, sspmp])
        # combine sspmp_df and h_df obsprefixes
        hobspref = h_df.obsprefix.unique().tolist()
        sspmp_obspref = sspmp.obsprefix.unique().tolist()
        all_obspref = hobspref + sspmp_obspref
        all_obspref = list(set(all_obspref))
        ...
        for prefix in uprefixes:
            ...
            if prefix in sspmp.obsprefix.unique():
                uh_df = sspmp.loc[sspmp.obsprefix==prefix,:].copy()
            else:
            ...
            # get pobs that contain prefix_shrt in obs.obsnme
            pobs = obs.loc[obs.obsnme.apply(lambda x: prefix_shrt in x),:].copy()
            if uh_df.otype.iloc[0] == "sspmp":
                pobs = pobs.loc[pobs.oname=="sspmphds",:]
            else:
            ...
            for k in uk:
                ...
                if ukobs.shape[0] == 0:
                    ...
                    continue
                for head,dt in zip(kuh_df.loc[:,'gwe_ft'],kuh_df.datetime):
                    #print(head, dt)
        ...
        master_targs = pd.read_csv(os.path.join('data','raw','water_lvl_targs_manual_ly_assign.csv'))
        master_targs = master_targs.reset_index(drop=True)
        master_targs = master_targs.rename(columns={'group number':'grpid',
                                                'manually_corrected_lay':'k',
                                                'row':'i',
                                                'col':'j'})
   
        # if any loc_id is nan, drop the row
        master_targs = master_targs.loc[master_targs['loc_id'].notna(), :].copy()

        # coerce k, i, j to numeric; anything bad becomes NaN
        for c in ['k', 'i', 'j']:
            master_targs[c] = pd.to_numeric(master_targs[c], errors='coerce')
        mask_bad = ~np.isfinite(master_targs[['k','i','j']]).all(axis=1)
        
        master_targs = master_targs.loc[~mask_bad].copy()
        
        master_targs[['k','i','j']] = (master_targs[['k','i','j']].astype('Int64'))
        
        master_targs = master_targs[['grpid','k','i','j','index_well_flag']]
        master_targs['k'] = master_targs['k']-1
        
        t_df_loc = t_df_loc.merge(master_targs, on=['grpid','k','i','j'], how='left')
        ...
        # set head difference obs:
        hdf_obs = pd.read_csv(os.path.join('data','analyzed','transient_well_head_diffs.csv'))
        unq_prefix = hdf_obs.columns[1:]
        oname_obsval_dict_diff = {}
        for prefix in unq_prefix:
            pfix = prefix.lower()
            df = hdf_obs.loc[hdf_obs[prefix].notnull(),['start_datetime',prefix]].copy()
            pobs = obs.loc[obs.obsnme.apply(lambda x: pfix in x),:].copy()
            
            dt_vals = df['start_datetime'].unique()
            pobs_w_meas = pobs.loc[pobs.obsnme.apply(lambda x: any(dt in x for dt in dt_vals)),:].copy()
            if pobs_w_meas.shape[0] == 0:
                print('empty obs for prefix:{0}'.format(prefix))
                continue
            for idx, row in pobs_w_meas.iterrows():
                oname = row.obsnme
                sim_date = oname.split(':')[-1]
                val = df.loc[df['start_datetime'] == sim_date, prefix].values[0]
                if np.isnan(val):
                    assert False, 'nan value for {0}, something went wrong...qa needed'.format(oname)
                oname_obsval_dict_diff.setdefault(oname, []).append(val)
        ...
        if flow_weight_scheme == 'all_wl_meas':
            for oname,vals in oname_obsval_dict.items():
                ...
                if int(obs.loc[oname,'k']) == 4: 
                    if 'oname:sspmphds' in oname:
                        obs.loc[oname,'obgnme'] = 'sswbvpmphds' # bc ss pmp obs only in wbv
                    else:
                        obs.loc[oname,'obgnme'] = 'sswbvhds'
                else:
                    if 'oname:sspmphds' in oname:
                        obs.loc[oname,'obgnme'] = 'sspmphds'
                    else:
            ...
            for oname,vals in oname_obsval_dict_trans.items():
                ...
                if year >= 1980:
                    if int(obs.loc[oname,'k']) == 4:
                        obs.loc[oname,'obgnme'] = 'wbvtranshds'
                    else:
                        obs.loc[oname,'obgnme'] = 'wstranshds'
                else:
                    obs.loc[oname,'obgnme'] = 'transearlyhds' 
                # get t_df_loc row that which has obsprefix in oname:
                for idx,r in t_df_loc.iterrows():
                    if r.obsprefix in oname:
                        tinfo = r
                        break
                assert tinfo is not None, 'tinfo not found, investigate'
                if tinfo['index_well_flag']:
                    obs.loc[oname,'obgnme'] = 'idx_well'
                    
                if thin_clustered_obs: 
                    to_drop = cluster.loc[cluster['index_well'] == False, 'id'].values
                    grpid = re.search(r"grpid:\d+_k:\d+", oname).group(0)
                    if grpid not in to_drop:
                        continue
                    obs.loc[oname,'obgnme'] = 'clustered'
                    obs.loc[oname,'weight'] = 0.0
                    obs.loc[oname,'observed'] = False
            ...
            for oname,vals in oname_obsval_dict_diff.items():
                vals = np.array(vals)
                year = int(oname.split(':')[-1].split('-')[0])

                obs.loc[oname,'obsval'] = vals
                obs.loc[oname,'observed'] = True
                obs.loc[oname,'standard_deviation'] = 3.0
                obs.loc[:,'count'] = len(vals)
                obs.loc[oname,'weight'] = 0.333
                obs.loc[oname,'obgnme'] = 'hdiff'
   
            #obs.loc[(obs.obgnme=="greater_rivflow") & (obs.obsnme.str.contains("1970-01-01")),'weight']=1.0
                        
        trobs = obs.loc[obs.oname=="transhds",:].copy()
        trobs['id'] = trobs.obsnme.apply(lambda x: x.split('transh_')[1].split('_i')[0])
        nzobs = trobs.loc[trobs.weight>0,:]
        groups = nzobs.groupby("id").count()
        groups = groups.loc[groups.obsnme>5,:].index
        for group in groups:
            gobs = nzobs.loc[nzobs["id"]==group]
            quantile = np.quantile(gobs.obsval,0.15)
            gobs = gobs.loc[gobs.obsval<=quantile,:]
            obs.loc[gobs.obsnme,"weight"] *= 5
            # manual dewighting:
            if group.startswith('grpid:147_k:'):
                obs.loc[gobs.obsnme,"weight"] = 0.0
            elif group.startswith('grpid:145_k:'):
                obs.loc[gobs.obsnme,"weight"] = 0.0

        #trobs["id"] = trobs["id"]
        #zero = trobs.loc[trobs["id"].apply(lambda x: x in [586,569]),"obsnme"]# problematic locations - update to flagged obs
        #obs.loc[zero,"weight"] = 0.0 
        trobs["k"] = trobs.k.astype(int)
        # if nlay == 6:
        #     zero = trobs.loc[trobs["k"]!=3,"obsnme"]
        # elif nlay == 7:
        #     zero = trobs.loc[trobs["k"]!=4,"obsnme"]
        #obs.loc[zero,"weight"] = 0.0
        nztrobs = trobs.loc[trobs.weight>0,:]
        obs.loc[nztrobs.loc[nztrobs.obsval>940,"obsnme"],"weight"] *= 2.0
        obs.loc[nztrobs.loc[nztrobs.obsval>940,"obsnme"],"weight"] *= 2.0
        
        # increase standard deviation for initial SS obs
        obs.loc[obs.obgnme== 'wbvsshds', 'standard_deviation'] = 3
        obs.loc[obs.obgnme== 'wsssshds', 'standard_deviation'] = 3
        obs.loc[obs.obgnme== 'wbvsshds', 'weight'] = 0.333
        obs.loc[obs.obgnme== 'wsssshds', 'weight'] = 0.333
        
        if kobs_greater_than:
            # Set less than obs to make sure sim heads stay below top surface
            ij_pairs = [(85,40),(92,42),(100,42),(108,46)]
            obsnmes = []
            for ij in ij_pairs:
                wbv_kh = obs.loc[obs.obsnme.str.contains('k_k:004')]
                wbv_pts = wbv_kh.loc[(wbv_kh.i==str(ij[0])) & (wbv_kh.j==str(ij[1])) ,'obsnme'].values[0]
                obsnmes.append(wbv_pts)
            
            assert len(obsnmes) == len(ij_pairs), 'kobs locs were not found, investigate'
            
            obs.loc[obs.obsnme.isin(obsnmes), 'weight'] = 1.0
            obs.loc[obs.obsnme.isin(obsnmes), 'obgnme'] = "greater_than_wbv_kh"
            obs.loc[obs.obsnme.isin(obsnmes), 'obsval'] = 50.0
            
            ij_pairs = [(22,44),(34,44),(45,41),(57,39),(68,37)]
            obsnmes = []
            for ij in ij_pairs:
                wbv_kh = obs.loc[obs.obsnme.str.contains('k_k:004')]
                wbv_pts = wbv_kh.loc[(wbv_kh.i==str(ij[0])) & (wbv_kh.j==str(ij[1])) ,'obsnme'].values[0]
            ...
            obs.loc[obs.obsnme.isin(obsnmes), 'weight'] = 1.0
            obs.loc[obs.obsnme.isin(obsnmes), 'obgnme'] = "greater_than_wbv_kh"
            obs.loc[obs.obsnme.isin(obsnmes), 'obsval'] = 25.0
...
def draw_noise_reals(t_d,modnm):
    ...
    #now for ss obs
    ss_groups = ["sswbvhds","wsssshds","sswbvpmphds", "sspmphds"]
    ...
    with PdfPages(os.path.join(t_d,"noise_draws.pdf")) as pdf:
        ...
        for uid in uids:
            ...
            fig,ax = plt.subplots(1,1,figsize=(10,10))
            ax.scatter(dts,uobs.obsval,marker="^",c="r",s=50)
            vals = oe.loc[:,uobs.obsnme].values
            [ax.plot(dts,vals[i,:],'r-',lw=0.5,alpha=0.5,zorder=10) for i in range(vals.shape[0])]
            ax.set_title(uobs.obgnme.iloc[0]+" "+uid,loc="left")
            ax.grid()
...
if __name__ == "__main__":
    ...
    phi_factor_dict = {'sswbvhds':1e-20,'wbvtranshds':0.4,'idx_well':0.3,'wstranshds':0.2,'sswbvpmphds':1e-20,'wsssshds':1e-20,
                        'sspmphds': 1e-20, 'transearlyhds': 0.20,'hdiff': 0.1,'ddtrgs':0.1,'greater_than_wbv_kh':0.1,}
    ...
    if high_dimensional:
        m_d_flow += '_highdim_noWR_final_rch_parm'
        t_d_flow += '_highdim_noWR_final_rch_parm'
    else:
        m_d_flow += '_lowdim'
        t_d_flow += '_lowdim'
    ...
    if prep_model:
        import wahp02_model_build_conf
        wahp02_model_build_conf.main()
        ...
        sim = flopy.mf6.MFSimulation.load('mfsim.nam', sim_ws=org_d, load_only=['dis'])
        gwf = sim.get_model(modnm)
        init_gw_vista_ss_mods(os.path.join('model_ws',f'{modnm}_clean'),mwd=modnm+run_tag,gwf=gwf,modnm=modnm)


        # reset org_d to gwv version:
        org_d = os.path.join('model_ws', modnm+run_tag)
        set_initial_array_vals(org_d, modnm=modnm, run_tag=True)
    ...
    if prep_flow:
        ...
        temp_dir = setup_pstpp(org_d,modnm,run_tag,t_d_flow,flex_con=False,num_reals=num_reals_flow,
                               high_dimensional=high_dimensional, tie_window_pars=tie_window_pars,)

        print(f'------- flow-ies has been setup in {t_d_flow} ----------')
        
        set_obsvals_and_weights(t_d_flow,modnm=modnm,flow_weight_scheme= 'all_wl_meas',
                                phi_factor_dict=phi_factor_dict,thin_clustered_obs=False,
                                kobs_greater_than=True) 
    ...
    if plot_flow:
       import wahp04_process_plot_results
       m_d = m_d_flow
       wahp04_process_plot_results.plot_simple_timeseries(m_d_flow,modnm=modnm,noptmax=noptmax_flow)
       wahp04_process_plot_results.plot_simple_1to1(m_d_flow, modnm=modnm)
       wahp04_process_plot_results.plot_simple_par_histo(m_d_flow, modnm=modnm)
       wahp04_process_plot_results.plot_phi_sequence(m_d_flow, modnm=modnm)
       wahp04_process_plot_results.plot_parm_violins(m_d_flow, f'{modnm}.pst', noptmax_flow)
       wahp04_process_plot_results.plot_simple_par_maps(m_d,modnm)
       obsdict = wahp04_process_plot_results.get_ies_obs_dict(m_d=m_d, pst=None, modnm=modnm)
       itrmx = max(obsdict)
       wahp04_process_plot_results.plot_water_budget(m_d, obsdict,pie_year=2022)
       wahp04_process_plot_results.plot_wateruse(m_d=m_d, modnm=modnm, noptmax=itrmx, max_fail=0)
       wahp04_process_plot_results.plot_zone_bud_ly_budget(m_d, obsdict)
       wahp04_process_plot_results.plot_zone_bud_ly_by_perctot(m_d, obsdict, top_n=6)
       wahp04_process_plot_results.plot_fancy_obs_v_sim(m_d,obsdict)

       wahp04_process_plot_results.base_posterior_param_forward_run(m_d0=m_d, noptmax=itrmx,modnm=modnm)
